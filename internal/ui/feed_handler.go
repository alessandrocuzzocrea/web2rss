...
func (h *Handler) handleNewFeed(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}

	h.renderNewFeed(w, nil)
}

func (h *Handler) handleDuplicateFeed(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	feed, err := h.queries.GetFeed(r.Context(), feedID)
	if err != nil {
	...
	h.renderNewFeed(w, data)
}

func (h *Handler) renderNewFeed(w http.ResponseWriter, data any) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")

	// Execute the template
	if err := h.templates.ExecuteTemplate(w, "new_feed.html", data); err != nil {
		// print the err
...
func (h *Handler) handlePreviewFeed(w http.ResponseWriter, r *http.Request) {
	...
	if existingSelectorIDStr != "" {
		...
		template_feed, err := h.queries.GetFeed(r.Context(), existingSelectorID)
		if err != nil {
	...
	ExistingSelectors, err := h.queries.ListFeeds(r.Context())
	if err != nil {
	...
	if err := h.templates.ExecuteTemplate(w, "feed-selector-partial.html", data); err != nil {
		// print the err
		fmt.Printf("Template error: %v\n", err)
		http.Error(w, "Template error", http.StatusInternalServerError)
		return
...
func (h *Handler) handleCreateFeed(w http.ResponseWriter, r *http.Request) {
	...
	_, err := h.queries.CreateFeed(r.Context(), db.CreateFeedParams{
		Name:          name,
		Url:           url,
...
func (h *Handler) handleEditFeed(w http.ResponseWriter, r *http.Request) {
	...
	// Execute the template
	if err := h.templates.ExecuteTemplate(w, "edit_feed.html", data); err != nil {
		// print the err
		fmt.Printf("Template error: %v\n", err)
		http.Error(w, "Template error", http.StatusInternalServerError)
		return
...
func (h *Handler) handleUpdateFeed(w http.ResponseWriter, r *http.Request) {
	...
	err = h.queries.UpdateFeed(r.Context(), db.UpdateFeedParams{
		ID:            feedID,
		Name:          name,
		Url:           url,
		ItemSelector:  sql.NullString{String: item_selector, Valid: item_selector != ""},
...
func (h *Handler) handleDeleteFeed(w http.ResponseWriter, r *http.Request) {
	...
	err = h.queries.DeleteFeed(r.Context(), feedID)
	if err != nil {
...
func (h *Handler) handleResetFeedItems(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	err = h.queries.DeleteItemsByFeedID(r.Context(), feedID)
	if err != nil {
...
func (h *Handler) handleRefreshFeed(w http.ResponseWriter, r *http.Request) {
	...
	err = h.feedService.RefreshFeed(r.Context(), feed)
	if err != nil {
